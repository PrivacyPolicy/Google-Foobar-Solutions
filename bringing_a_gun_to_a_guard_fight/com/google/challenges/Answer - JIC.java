...
public class Answer {
    ...
    public double[][] matrixSetup(double[] a, double[] b, int[] walls) {
        int l = walls.length;
        int n = 0; // 1 if wall is reflect y else 0 for reflect x
        ...
        double[] row1 = matrixSetupRow(a[n],
                                       yForWall(walls[0]),
                                       yForWall(walls[1]));
        matrix[0][0] = row1[1];
        matrix[0][1] = row1[2];
        matrix[0][l] = -a[0] * row1[0];
        double[] row;
        for (int i = 0; i < l - 2; i++) {
            row = matrixSetupRow(yForWall(walls[i]),
                                 yForWall(walls[i + 1]),
                                 yForWall(walls[i + 2]));
            matrix[i + 1][i] = row[0];
        ...
        double[] lastRow = matrixSetupRow(yForWall(walls[l - 2]),
                                          yForWall(walls[l - 1]),
                                          b[n]);
    ...
    public double xForWall(int wall) {
    ...
    public static void main(String[] args) {
        ...
        //
        //
        int[] walls = new int[] {a.WALL_LEFT, a.WALL_RIGHT, a.WALL_LEFT};
